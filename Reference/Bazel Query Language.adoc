...
句法上，一个target pattern就是一个word。这个word变成一个target的集合set。最简单的target pattern就是一个label，这个label标记一个单一target。比如，//foo:bar是一个集合，里面只有一个target，就是bar这个rule。
...
target pattern接受通配符wildcards在package和target上。比如...，all
